...
const SansTrans = {
  ...
  makeMap(from, to, options) {
        var alternates = this.allAlternates[from] || {},
            consonants = {},
  ...
  t(data, from, to, options) {
    options = options || {};
    let cachedOptions = this.cache.options || {},
        defaults = this.defaults,
        hasPriorState = (this.cache.from === from && this.cache.to === to),
        map;
  ...
  iast(input){
    return(this.t(input, "devanagari", "iast"))
  },

  dev(input){
    return(this.t(input, "iast", "devanagari"))
  },
  ...
  init(){
    ...
    let kolkata = this.schemes.kolkata = this.cheapCopy(this.schemes.iast),
